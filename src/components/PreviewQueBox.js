...
const PreviewQueBox = ({ data, order }) => {
	const [position, setPosition] = useState();
	const handleChange = (position) => {
		setPosition(position)
	};
	const { queArr, setQueArr } = useUserQuiz();
	...
	return (
		<form className='relative p-6 pt-0 flex-auto mb-6 border-2 rounded-lg'>
			<div className="relative m-auto my-6 flex justify-between items-center">
				...
				<div onClick={() => { deleteElem(order) }}><BiTrash className='text-xl cursor-pointer'/></div>
			</div>
